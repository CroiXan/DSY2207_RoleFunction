...
public class UserRolesFunctions {
    ...
    public HttpResponseMessage crearUserRole(
        ...
        HttpRequestMessage<Optional<UserRoles>> request,
        final ExecutionContext context) {

        UserRoles userRol = request.getBody().orElse(null);
        if (userRol == null) {
    ...
    public HttpResponseMessage obtenerUserRolesPorId(
        @HttpTrigger(name = "req", methods = {HttpMethod.GET}, route = "getUserRole/{id}", authLevel = AuthorizationLevel.FUNCTION)
        ...
        try {
            ...
            context.getLogger().severe(e.getMessage());
            return request.createResponseBuilder(HttpStatus.INTERNAL_SERVER_ERROR).body("Error al consultar Rol de usuario").build();
        }
    }

    @FunctionName("getUserRoleByUser")
    public HttpResponseMessage obtenerUserRolesPorUser(
        @HttpTrigger(name = "req", methods = {HttpMethod.GET}, route = "getUserRoleByUser/{id}", authLevel = AuthorizationLevel.FUNCTION)
        HttpRequestMessage<Optional<String>> request,
        @BindingName("id") String id,
        final ExecutionContext context) {
    ...
    public HttpResponseMessage obtenerRolesPorRolId(
        @HttpTrigger(name = "req", methods = {HttpMethod.GET}, route = "getUserRoleByRoleId/{id}", authLevel = AuthorizationLevel.FUNCTION)
        HttpRequestMessage<Optional<String>> request,
        @BindingName("id") String id,
        final ExecutionContext context) {
    ...
    public HttpResponseMessage eliminarUserRoles(
        @HttpTrigger(name = "req", methods = {HttpMethod.DELETE}, route = "deleteUserRole/{id}", authLevel = AuthorizationLevel.FUNCTION)
        HttpRequestMessage<Optional<String>> request,
        @BindingName("id") String id,
        final ExecutionContext context) {
